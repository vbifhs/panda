...
const CanMsg TOYOTA_TX_MSGS[] = {{0x283, 0, 7}, {0x2E6, 0, 8}, {0x2E7, 0, 8}, {0x33E, 0, 7}, {0x344, 0, 8}, {0x365, 0, 7}, {0x366, 0, 7}, {0x4CB, 0, 8},  // DSU bus 0
                                 ...
                                 {0x791, 0, 8}}; // DSU UDS TX addr Bus 0 (for DSU disable)
...
static int toyota_tx_hook(CANPacket_t *to_send) {
  ...
  // UDS: Only tester present ("\x02\x3E\x00\x00\x00\x00\x00\x00\x00") allowed on diagnostics address
  if (addr == 0x791) {
    if ((GET_BYTES(to_send, 0, 4) != 0x00003E02U) || (GET_BYTES(to_send, 4, 4) != 0x0U)) {
      tx = 0;
    }
  }
