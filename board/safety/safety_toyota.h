...
static int toyota_fwd_hook(int bus_num, int addr) {
  ...
  if (bus_num == 2) {
    ...
    int is_dsu_diag_resp_msg = (addr == 0x798);
    // in TSS2 the camera does ACC as well, so filter 0x343
    int is_acc_msg = (addr == 0x280) || (addr == 0x689);
    int block_msg = is_lkas_msg || is_dsu_diag_resp_msg || (is_acc_msg && !toyota_stock_longitudinal);
    if (!block_msg) {
      bus_fwd = 0;
    }
  }
